...
**Version:** 1.1 (M2 — Stripe Customer Portal data added to §2; DPA template URL bound)
...
**Cross-references:** `legal/terms-of-service.md`, `legal/aup.md`, `legal/subprocessors.md`, `legal/data-processing-agreement.md` (Art. 28 DPA — LIVE at M2), `marketing/analytics-spec.md` (event taxonomy), `architecture/system-diagram.md` (subprocessor flow), `finance/refund-matrix.md` §5 (CCPA surface)
...
| **Stripe Customer Portal data**        | Billing history (invoices, payments, refunds), payment-method last-4 + brand + expiry month/year, subscription change-log, cancellation timestamp; all rendered in the Stripe-hosted Customer Portal at M2 per `finance/stripe-config.md` §1.5 | Surface the FTC Click-to-Cancel UX (`compliance/click-to-cancel-ux-audit.md`); let you update card / view receipts | Art. 6(1)(b) contract performance + 6(1)(c) legal obligation (tax + cancellation record-keeping)                            |
...
- a System Card at `/system-card` (see `legal/ai-system-card-v0.5.md`).
...
_Comply locks this Privacy Policy at v1.1 on 2026-05-12 (M2 — Stripe Customer Portal data class added; DPA template URL bound). Re-verify quarterly. Any regulatory change (EU AI Act Art. 50 binding 2026-08-02; CPRA regulation updates; Anthropic data-handling posture changes for commercial customers; Supabase or other subprocessor data-residency changes) triggers a version bump._

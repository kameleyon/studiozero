...
> **Method.** Components, trust boundaries, and data flows derived from PRD v0.5 §6 (surface), §7.2 (audit workflow), §8 (three modes), §11 (Auto-PR V1.5), §13 (architecture), §14 (NFRs incl. retention + a11y + AUP), §16 (milestones + exit gates), §17 (decisions D1–D23). Cross-phase decisions in `shared_context/projects/studio-zero-productization/decisions.md` (IA-D1/D2/D3) and `architecture/decisions.md` (ARCH-D1–D8). Every contradiction surfaced during drafting is named in the _Internal contradictions_ section at the end.

---

## TB-numbering reconciliation (Jury F-CRIT-1, M0 close)

This diagram historically used Axiom's component-flow TB-N labels (TB-0..TB-10). **Shield's `threat-model.md` is the canonical STRIDE register and uses TB-1..TB-15.** Where TB-N appears in this diagram, the canonical Shield TB is named in parentheses. Verify's tests and any other cross-document reference resolve against Shield's canonical numbering.

| Axiom-flow label (this diagram) | Shield canonical (threat-model.md)             | Boundary description                              |
| ------------------------------- | ---------------------------------------------- | ------------------------------------------------- |
| TB-0 (public)                   | — (not in STRIDE register; non-trust-boundary) | Cloudflare CDN / marketing static                 |
| TB-1                            | TB-1                                           | Customer browser → Web App                        |
| TB-2                            | TB-3                                           | Web App → Job Queue → Hosted Runner               |
| TB-3                            | TB-2                                           | Web App + Runner → Supabase (Vault subset)        |
| TB-4                            | TB-4 + TB-6                                    | Hosted Runner → Anthropic + Deployed URL (egress) |
| TB-5                            | TB-4                                           | Hosted Runner → Anthropic API                     |
| TB-6                            | TB-5                                           | Hosted Runner → GitHub repo clone                 |
| TB-7                            | TB-7                                           | Web App ↔ CLI Companion                          |
| TB-8 (Stripe webhook)           | TB-11                                          | Stripe → Web App (webhook delivery)               |
| TB-9 (telemetry SaaS)           | TB-14                                          | Hosted Runner → Sentry / PostHog / Resend         |
| TB-10 (GitHub webhook)          | TB-13                                          | GitHub → Web App (webhook delivery)               |

Full renumbering to Shield's canonical TB-1..TB-15 is M1 cleanup work (low-risk doc edit, deferred to keep M0 close).
...
_End of system-diagram v0.1. Axiom — Strategy Layer._

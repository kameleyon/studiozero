...
**Phase:** 7 — Pricing & Unit Economics (Phase 9 M2 Batch 2 amendment)
**Owner:** Meter (FinOps) — peer-coordinated with Penny (pricing)
**Version:** 0.2
**Date:** 2026-05-12 (Phase 9 M2 Batch 2 amendment: §6 marked LIVE; per-plan cap defaults locked at Managed Starter $15 + Managed Pro $80 per #15.4)
**Status:** Phase-7 exit gate cleared; §6 R1 mitigation now LIVE per M2 Batch 1/2 deliveries (Atlas f67ef63 + Forge a7396fc + Meter monitoring spec).
...
**Status (Phase 9 M2 Batch 2, 2026-05-12): LIVE.**

- Atlas wired the per-tenant usage table + cap-check function in `architecture/database/migrations/0003_billing_managed.sql` §C (`tenant_token_usage_daily` + `check_token_budget(uuid)`; commit `f67ef63`).
- Forge wired `check_token_budget()` into the LLM gateway pre-flight at `supabase/functions/llm-gateway/index.ts` step 5 (commit `a7396fc`); on FALSE the gateway returns HTTP 429 `token_budget_exceeded` and the runner falls back to `/app/billing/token-budget-exceeded`.
- Meter wired the monitoring layer (70%/90% Sentry warnings via the poller, 110% hard-block already enforced) + the cost-per-tenant dashboard + the on-call runbook. See `finance/r1-token-cap-monitoring.md`, `finance/cost-per-tenant-dashboard.md`, `finance/budget-alert-runbook.md`.

**Per-tenant cap defaults (LOCKED per Phase 9 M2 Batch 2 brief #15.4):**

| Plan                | `tenants.token_budget_micros` | USD/mo                                                                                                                                                        |
| ------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Free (Surface only) | `0`                           | $0 — defense-in-depth; Surface free-tier doesn't go through the Managed path anyway                                                                           |
| BYOK Starter        | `NULL`                        | n/a — customer pays Anthropic; `check_token_budget()` returns true on NULL                                                                                    |
| BYOK Pro            | `NULL`                        | same                                                                                                                                                          |
| **Managed Starter** | `15000000`                    | **$15.00 / mo** — note this is the original `unit-economics.md` v0.1 §6 recommendation, NOT a recompute                                                       |
| **Managed Pro**     | `80000000`                    | **$80.00 / mo** — note this is an M2-brief revision of v0.1's $45 recommendation; rationale + sensitivity analysis in `finance/r1-token-cap-monitoring.md` §5 |
| CLI                 | `NULL`                        | n/a — customer pays Anthropic via their Claude Code subscription                                                                                              |

For the operational response when caps breach, see `finance/budget-alert-runbook.md`.
...
- **Managed Starter:** hard cap at **$15 of Anthropic spend / tenant / month** (allows ~9 Comprehensive audits at modeled rate, ~3 at worst case, leaving 84%+ margin floor). UI degrades gracefully per Penny's prd-review §3.5: _"plug in a BYOK key to continue for the rest of the month."_ Locks PRD §19 R1 risk closed.
...
**Where the cap fires in code (M2 actuals):** Phase 9 M2 Batch 1 moved the cap-check from the JWT-mint surface (the v0.1 design) to the **LLM gateway pre-flight** — Forge's `supabase/functions/llm-gateway/index.ts` step 5 calls Atlas's `check_token_budget(tenant_id)` SECURITY DEFINER function BEFORE each Anthropic call. The function reads month-to-date `SUM(cost_micros)` from `tenant_token_usage_daily` and returns FALSE iff used ≥ cap × 1.10. The 10% headroom is the documented R1 grace (`unit-economics.md` v0.1 §6 + `0003_billing_managed.sql` §C comment) — we lose 10% of margin rather than hard-cut a customer mid-audit. UI degrades to `/app/billing/token-budget-exceeded`. Cap enforced **before** the Anthropic call, not after — closes the "kill it once it's burning" failure mode.

The Managed Pro cap was revised from v0.1's $45 to $80 in the M2 brief §15.4 — see the §6 LIVE-status table above and `finance/r1-token-cap-monitoring.md` §5 for the margin-sensitivity rationale (at $80 cap × 1.10 = $88 worst-case Anthropic spend, on $249 Managed Pro: net margin floor ≈ 68% after Stripe + Railway + Storage, above PRD §15 ≥80% target only in the modeled-tokens case; the cap is the floor that holds the ≥80% modeled margin even when audits hit the 295k-token midpoint).
...
| Managed Starter $99                 | 93.7% (at modeled tokens) — 91.6% (at 500k-token audits) — DEPENDS ON CAP (§6) at worst case | ✓ (target ≥80%) **§6 cap LIVE in M2 — condition cleared 2026-05-12** |
...
1. **§6 token cap MUST ship in M2** alongside Stripe + Managed mode. PRD §13.5 already requires it; this doc named the original $15/$45 numbers; the M2-brief revised Managed Pro to $80. **STATUS (2026-05-12): LIVE.** Atlas's `tenant_token_usage_daily` + `check_token_budget()` are in `0003_billing_managed.sql` (f67ef63); Forge's gateway pre-flight calls the function (a7396fc); Meter's monitoring + alerting + runbook ship in this Batch 2. **Phase-7 condition 1: CLEARED.**
...
_End of unit-economics.md v0.1. Meter — DevOps Layer._

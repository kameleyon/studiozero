...
**Owner:** Ledger
**Status:** **M2 LIVE** — §1-§9 are operationally executable. Forge code shipped Batch 1 (a7396fc); Atlas 0003 shipped (f67ef63); M2 Batch 2 ships the operational runbooks cross-referenced in §10. Go-live pending Jo's account provisioning per `finance/stripe-products-provisioning.md`.
**Coordinates with:** Penny (`finance/pricing.md` v1.0 locked), Comply (regional refund matrix D20, Dispute Finding path, `finance/refund-matrix.md` v1.0 locked), Forge (M2 Batch 1 shipped), Atlas (`subscriptions`, `billing_events`, `cooling_off_windows`, `disputes` migrations — 0003 shipped).
...
## 10. Operational runbooks (M2 Batch 2 — Ledger shipped)

Sections §1-§9 above are the **spec**. The 5 runbooks below are the **operationally executable** procedures Jo follows to take this spec live. Each runbook is line-by-line executable in Stripe Dashboard or via Stripe API.

- **`finance/stripe-products-provisioning.md`** — every product + price + metadata field + env-var name; Jo's checklist to provision Stripe products in Test mode → Live mode. Includes §3 env-var fill-in template for Vercel.
- **`finance/stripe-customer-portal-config.md`** — Customer Portal feature toggles + branding + Herald-locked copy + return URL; FTC Click-to-Cancel-compliant one-click cancel.
- **`finance/dunning-policy-config.md`** — Smart Retries config (3 retries over 21d) + email cadence triggers (T+0/+3/+7/+14/+21) + service-degradation rules during `past_due` + regional cancellation grace.
- **`finance/stripe-tax-config.md`** — Stripe Tax in monitoring mode at M2; per-region collection-mode flip plan; B2B VAT reverse-charge via VIES; per-state US sales-tax nexus monitoring.
- **`finance/refund-operations-runbook.md`** — operational procedure for every refund trigger (cooling-off auto / SB 313 auto / dispute manual / chargeback auto-by-Stripe / failed-synth auto / Auto-PR-rejection auto V1.5); idempotency key construction; audit-log canonical shape.

**M2 Batch 2 close:** Stripe go-live ready pending Jo's account provisioning per `stripe-products-provisioning.md`. After Jo executes that runbook end-to-end (90 min Test mode → Verify integration suite green → 45 min Live mode mirror), §1-§9 transition from "spec" to "live in production".

## 10a. Cadence note — dunning emails

§5.2 above lists email cadence as T+0/+3/+10/+17/+21. The brief and `finance/dunning-policy-config.md` §2 lock the cadence to **T+0/+3/+7/+14/+21**. The dunning-policy-config.md cadence supersedes — Verify aligns test fixtures, Trace updates `ia/user-flows/billing-and-cancel.md` EC-4 to T+0/+3/+7/+14/+21.

## 10b. Open items (still open after M2 Batch 2)

- **D4 (Penny + Jo, M2 ticket-cut week 7):** $19 vs $29 Starter pricing — both Price objects pre-created in `stripe-products-provisioning.md` §1.1; Forge does not hard-code prices; A/B harness reads `subscriptions.metadata.ab_slot`.
- **D5 (Penny + Jo, V1.5 spec-kickoff week 17):** Auto-PR flat $49 vs tiered $15/$49/$99 — Product `prod_studiozero_autopr_fix` ready; tiered Price objects can be added at V1.5 without product re-creation per provisioning §1.6.
- **FTC cancel-with-refund button (Comply, M2):** PRD § leaves ambiguous whether non-CA US customers should get a "cancel AND refund" path; 16 CFR 425 mandates easy cancel, not refund. Comply confirms; if "no refund outside regional law" stands, `refund-operations-runbook.md` §1 row `ftc_cancel_with_refund` stays manual-only.
- **`stripe_reconcile_poll` enum value (Atlas, M2):** add to `audit_action` enum in `0003_billing_managed.sql` per §4.2 of this doc.
- **`refund_issued` enum value (Atlas, M2 Batch 3):** add to `audit_action` enum per `refund-operations-runbook.md` §7.
- **`/api/admin/refunds/execute` endpoint (Forge, M2 Batch 3):** implement per `refund-operations-runbook.md` §6.1 with RBAC + server-side idempotency-key derivation.
- **Legal entity placeholder (Comply, M2):** Stripe account holder + statement descriptor wording pending trademark + entity resolution. Blocks `stripe-tax-config.md` §2 origin-address fill-in.
- **EU/UK Price objects in EUR/GBP (Penny + Locale, post-MVP):** added when first EU/UK customer signals; pricing-page renders converted USD until then with the disclosure "Billed in USD; your bank may apply FX fees." `priceEnvVarName()` in Forge's checkout-session route needs a `currency` parameter at that time.

---

## 11. Stripe Connect — deferred to V2+

**Decision:** Stripe Connect (marketplace billing) is **NOT** wired at MVP, V1.5, or V2 unless a future use case emerges.

**Rationale:** Stripe Connect is Stripe's marketplace product — the platform pays OUT to connected accounts (sellers, contractors, payees). Studio Zero's current and roadmap-mapped use cases involve customers paying Studio Zero for a service Studio Zero delivers. We do not pay out to customers; we do not facilitate a marketplace; we are not a Connect platform.

**If this changes** (e.g., V2+ adds a "verified auditor marketplace" SKU where third-party reviewers earn revenue share, or a "white-label reseller" model — though PRD §4 currently lists white-label as explicit non-goal): the Stripe Connect onboarding spec lives outside this document and outside Ledger's MVP/V2 owned scope. Re-open this section then.

**Code consequence:** no Connect-related code paths in Forge's M2 Batch 1 (`/api/billing/checkout-session/route.ts`, `/api/webhooks/stripe/route.ts`). No `transfer_data`, no `application_fee_amount`, no `on_behalf_of` parameters in Stripe Checkout Session creation. Confirmed by reading `apps/web/app/api/billing/checkout-session/route.ts:214-245` — no Connect params present.
...
- **`finance/stripe-products-provisioning.md`** (M2 Batch 2; Jo's provisioning runbook)
- **`finance/stripe-customer-portal-config.md`** (M2 Batch 2; Customer Portal toggles + branding + URLs)
- **`finance/dunning-policy-config.md`** (M2 Batch 2; Smart Retries + 5-touchpoint cadence)
- **`finance/stripe-tax-config.md`** (M2 Batch 2; monitoring mode → collection mode)
- **`finance/refund-operations-runbook.md`** (M2 Batch 2; per-trigger operational procedure)

---

_Owner: Ledger · Phase 7 spec (§1-§9) + Phase 9 M2 Batch 2 ops (§10-§11) · Last update: 2026-05-12 · Status: M2 LIVE pending Jo's Stripe account provisioning per `stripe-products-provisioning.md`_
